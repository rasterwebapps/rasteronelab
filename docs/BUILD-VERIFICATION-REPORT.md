...
| lis-sample | `backend/lis-sample/build.gradle.kts` | Service | `org.springframework.boot`, `java` | `com.rasteronelab.lis.sample.LisSampleApplication` (line 28) | ✅ PASS | ⚠️ UNKNOWN | **No application.yml** — will use Spring defaults (port 8080). Needs config file. Port unassigned. |
...
| lis-result | `backend/lis-result/build.gradle.kts` | Service | `org.springframework.boot`, `java` | `com.rasteronelab.lis.result.LisResultApplication` (line 26) | ✅ PASS | ⚠️ UNKNOWN | **No application.yml** — will use Spring defaults. Port unassigned. |
| lis-report | `backend/lis-report/build.gradle.kts` | Service | `org.springframework.boot`, `java` | `com.rasteronelab.lis.report.LisReportApplication` (line 38) | ✅ PASS | ⚠️ UNKNOWN | **No application.yml** — will use Spring defaults. Port unassigned. Needs MinIO for storage. |
...
| lis-inventory | `backend/lis-inventory/build.gradle.kts` | Service | `org.springframework.boot`, `java` | `com.rasteronelab.lis.inventory.LisInventoryApplication` (line 26) | ✅ PASS | ⚠️ UNKNOWN | **No application.yml**, **no Flyway migrations**, **no resources dir**. Scaffold only. |
| lis-instrument | `backend/lis-instrument/build.gradle.kts` | Service | `org.springframework.boot`, `java` | `com.rasteronelab.lis.instrument.LisInstrumentApplication` (line 29) | ✅ PASS | ⚠️ UNKNOWN | **No application.yml**, **no resources dir**. Has HAPI FHIR deps. Scaffold only. |
| lis-qc | `backend/lis-qc/build.gradle.kts` | Service | `org.springframework.boot`, `java` | `com.rasteronelab.lis.qc.LisQcApplication` (line 26) | ✅ PASS | ⚠️ UNKNOWN | **No application.yml**, **no resources dir**. Scaffold only. |
...
| lis-notification | `backend/lis-notification/build.gradle.kts` | Service | `org.springframework.boot`, `java` | `com.rasteronelab.lis.notification.LisNotificationApplication` (line 33) | ✅ PASS | ⚠️ UNKNOWN | **No application.yml**, **no resources dir**. Has Twilio + Thymeleaf deps. Scaffold only. |
| lis-integration | `backend/lis-integration/build.gradle.kts` | Service | `org.springframework.boot`, `java` | `com.rasteronelab.lis.integration.LisIntegrationApplication` (line 29) | ✅ PASS | ⚠️ UNKNOWN | **No application.yml**, **no resources dir**. Has HAPI FHIR client. Scaffold only. |
...
| 3 | **8 services missing `application.yml`**: lis-sample, lis-result, lis-report, lis-inventory, lis-instrument, lis-qc, lis-notification, lis-integration have no runtime config — will fail to connect to DB/Redis/Keycloak | HIGH | ❌ OPEN | Each module's `src/main/resources/` directory |
...
| 9 | **Duplicate port conflict**: Services without config default to port 8080 (same as gateway). Running multiple unconfigured services simultaneously will cause bind failures | MEDIUM | ❌ OPEN | Missing application.yml in 8 modules |
...
1. **Missing `application.yml` for 8 services**: Are lis-sample, lis-result, lis-report, lis-inventory, lis-instrument, lis-qc, lis-notification, lis-integration intentionally left without config (scaffold-only), or should they have `application.yml` files with port assignments and service config?
...
3. **Port assignments for unconfigured services**: What ports should the remaining services use? Suggested: sample=8083, result=8084, report=8085, inventory=8087, instrument=8088, qc=8089, notification=8091, integration=8092 (to match gateway routing config).
